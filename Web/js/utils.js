import {
  ...
  import { getProductoss, productobyID, checkoutProductos, createComanda } from "./comunicationManager.js";
    ...
    setup() {
      ...
      function añadirALaCesta(index) {
        ...
        const nuevoProducto = {
          ...
          colors: productoSeleccionado.colors, 
          cantidad: 1,
          talla: selectedCombinacion.value.split(" ")[1],
          color: selectedCombinacion.value.split(" ")[0],
      };
      
    
        const productoEnCesta = productosEnCesta.value.find(
            (producto) => producto.id === productoSeleccionado.id && producto.color === nuevoProducto.color && producto.talla === nuevoProducto.talla
        );
      ...
      function getProducte(index) {
        if (index >= 0 && index < productos.value.length) {
          ...
          productobyID({ idProducto }).then(detallesProducto => {
            if (detallesProducto && Array.isArray(detallesProducto)) {
              ...
              productos.value[index].colorTallas = detallesProducto.map(item => `${item.Color} ${item.TallaZapato}`);
              console.log("Detalles del producto:", detallesProducto); // Log para verificar
            } else {
